...
# Update main .env with dispute monitor variables
update_main_env_with_dispute_vars() {
    log_info "Updating main .env with dispute monitor variables..."
    ...
    # Append to main .env file if not already present
    if [ -f ".env" ]; then
        # Remove old values if they exist
        sed -i.bak '/^ROLLUP_RPC=/d; /^PROPOSER_ADDRESS=/d; /^CHALLENGER_ADDRESS=/d; /^GAME_FACTORY_ADDRESS=/d' .env 2>/dev/null || true
        
        # Append new values
        cat >> .env << EOF

# Dispute Monitor Configuration (auto-generated by setup script)
ROLLUP_RPC=http://op-node:9545
PROPOSER_ADDRESS=$PROPOSER_ADDRESS
CHALLENGER_ADDRESS=$CHALLENGER_ADDRESS
GAME_FACTORY_ADDRESS=$GAME_FACTORY_ADDRESS
EOF
        log_success "Main .env updated with dispute monitor variables"
    else
        log_warning "Main .env file not found, skipping update"
    fi
}
...
main() {
    ...
    # Add dispute-monitor variables to main .env for docker-compose
    update_main_env_with_dispute_vars
