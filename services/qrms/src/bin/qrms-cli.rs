...
struct StatusResponse {
    ...
    apqc: ApqcStatus,
    #[serde(default)]
    aegis_tee: Option<AegisTeeStatus>,
    sequencer: SequencerStatus,
    chain: ChainStatus,
}

#[derive(Debug, Clone, Deserialize)]
struct AegisTeeStatus {
    platform: String,
    mrenclave: String,
    mempool_size: usize,
    asset_protection_enabled: bool,
    ...
    fn handle_event(&mut self, event: WsEvent) {
        match event {
            WsEvent::QrmUpdate { indicator, risk } => {
                ...
                // Log recommendation if rotation is needed
                if risk.recommendation != "continue" {
                    self.log(
                        LogLevel::Rotation,
                        "QRM",
            ...
            WsEvent::TxsOrdered { count, txs } => {
                self.total_txs += txs.len() as u64;
                self.pending_txs.extend(txs);
                if self.pending_txs.len() > 100 {
                    self.pending_txs.drain(0..self.pending_txs.len() - 100);
                }
                self.log(
                    LogLevel::Info,
                    "SEQ",
                    format!("Ordered {} transactions", count),
                );
            }
            ...
            WsEvent::QvmCircuitUpdate(update) => {
                ...
                if let Some(result) = update.result {
                    ...
                    self.log(
                        ...
                        "QVM",
                        format!(
                            "Circuit {} completed | reps={} | fidelity={:.2} | time={:.1}ms | outcomes={}",
                            result.circuit_id,
                            result.repetitions,
...
fn render_stats(f: &mut Frame, app: &App, area: Rect) {
    ...
    let stats = Line::from(vec![
        ...
        Span::raw(" │ "),
        Span::styled(
            format!("RISK: {:>5}", app.current_risk),
        ...
        Span::raw(" │ "),
        Span::styled(
            format!("ROT: {:>2}", app.rotations),
            Style::default().fg(if app.rotations > 0 {
                Color::LightRed
            } else {
                Color::DarkGray
            }),
        ...
        Span::raw(format!(
            "HEIGHT: {:>6}",
            app.status.as_ref().map(|s| s.chain.height).unwrap_or(0)
        )),
        Span::raw(" │ "),
        Span::styled(
            format!("CIRCUITS: {:>3}", app.total_circuits),
...
fn render_qvm(f: &mut Frame, app: &App, area: Rect) {
        ...
        if let Some(ref qvm) = status.qvm {
            vec![
                ...
                Line::from(vec![
                    ...
                    Span::styled(
                        format!("{:>5}", qvm.qrm_risk_score),
                        Style::default().fg(if qvm.qrm_risk_score < 3000 {
                            Color::Green
                        } else if qvm.qrm_risk_score < 6000 {
                            Color::Yellow
                        } else {
                            Color::Red
                        }),
                    ...
                    Span::styled(
                        format!("{:>5}", qvm.oracle_risk_score),
                        Style::default().fg(if qvm.oracle_risk_score < 3000 {
                            Color::Green
                        } else if qvm.oracle_risk_score < 6000 {
                            Color::Yellow
                        } else {
                            Color::Red
                        }),
    ...
    let status_para = Paragraph::new(qvm_status).block(
        Block::default()
    ...
    if let Some(circuit) = app.active_circuits.last() {
        ...
        let empty = Paragraph::new("No active circuits").block(
            Block::default()
    ...
    // Grover threats
    let grover_items: Vec<ListItem> = app
        .grover_threats
        .iter()
        .take(6)
        .map(|t| {
            ...
            ListItem::new(vec![
                ...
                Line::from(vec![
                    Span::raw("  "),
                    Span::styled(
                        format!("{} qubits", t.required_physical_qubits),
                        Style::default().fg(Color::Yellow),
                    ),
                    Span::raw(" │ "),
                    Span::styled(
                        format!("{:.1} years", t.estimated_time_years),
    ...
    let grover_list = List::new(grover_items).block(
        Block::default()
    ...
    // Shor threats
    let shor_items: Vec<ListItem> = app
        .shor_threats
        .iter()
        .take(6)
        .map(|t| {
            ...
            ListItem::new(vec![
                ...
                Line::from(vec![
                    ...
                    Span::raw(" │ "),
                    Span::styled(
                        format!("{:.1} hours", t.estimated_time_hours),
                        Style::default().fg(Color::DarkGray),
                    ),
                ]),
                Line::from(vec![
    ...
    let shor_list = List::new(shor_items).block(
        Block::default()
    ...
    let result_items: Vec<ListItem> = app
        ...
        .take(5)
        .map(|r| {
            let total_outcomes: usize = r.histogram.values().sum();
            let top_outcomes: Vec<_> = r
                .histogram
                .iter()
                .take(3)
            ...
            ListItem::new(vec![
                Line::from(vec![
                    Span::styled(&r.circuit_id, Style::default().fg(Color::Cyan)),
                    Span::raw(format!(
                        " | reps={} | outcomes={}",
                        r.repetitions,
                        r.histogram.len()
                ...
                Line::from(vec![
                    Span::raw("  Top: "),
                    Span::raw(top_outcomes.join(", ")),
                    Span::raw(format!(" | total={}", total_outcomes)),
                ]),
                Line::from(vec![Span::raw(format!(
                    "  Fidelity: {:.2}% | Time: {:.1}ms",
                    r.fidelity_estimate * 100.0,
                    r.execution_time_ms
                ))]),
            ])
        })
    ...
    let result_list = List::new(result_items).block(
        Block::default()
            .title(format!(" Recent Results ({}) ", app.circuit_results.len()))
            .borders(Borders::ALL)
            .border_style(Style::default().fg(Color::Magenta)),
    );
...
fn render_circuit(f: &mut Frame, circuit: &QuantumCircuit, area: Rect) {
    ...
    for q in 0..qubits_to_show {
        ...
        for moment in 0..=max_moment.min(50) {
            let gates_in_moment: Vec<_> = circuit
                .gates
                .iter()
    ...
    let circuit_para = Paragraph::new(lines).block(
        Block::default()
...
fn render_qrm(f: &mut Frame, app: &App, area: Rect) {
    ...
    // Top: QRM Status Summary
    let qrm_status = if let Some(ref status) = app.status {
        let risk_color = if status.qrm.risk_score < 3000 {
            Color::Green
        } else if status.qrm.risk_score < 6000 {
            Color::Yellow
        } else {
            Color::Red
        };
        let rec_color = match status.qrm.recommendation.as_str() {
            "continue" => Color::Green,
            "schedule_rotation" => Color::Yellow,
            "emergency_rotation" => Color::Red,
            _ => Color::White,
        };
        vec![
            Line::from(vec![
                ...
                Span::styled(
                    format!("{:>5}", status.qrm.risk_score),
                    Style::default().fg(risk_color).add_modifier(Modifier::BOLD),
                ),
                Span::raw(" │ "),
                Span::styled(" Recommendation: ", Style::default().fg(Color::Cyan)),
                Span::styled(&status.qrm.recommendation, Style::default().fg(rec_color)),
            ]),
            Line::from(vec![
                Span::styled(" Indicators: ", Style::default().fg(Color::Cyan)),
                Span::raw(format!("{:>4}", status.qrm.indicator_count)),
                Span::raw(" │ "),
                Span::styled(" Scheduled Threshold: ", Style::default().fg(Color::Cyan)),
                Span::styled(
                    format!("{:>5}", status.qrm.thresholds.scheduled),
                    Style::default().fg(Color::Yellow),
                ),
                Span::raw(" │ "),
                Span::styled(" Emergency Threshold: ", Style::default().fg(Color::Cyan)),
                Span::styled(
                    format!("{:>5}", status.qrm.thresholds.emergency),
    ...
    let status_para = Paragraph::new(qrm_status).block(
        Block::default()
            .title(" QRM Status ")
            .borders(Borders::ALL)
            .border_style(Style::default().fg(Color::Magenta)),
    ...
    // Bottom: Split into category breakdown and indicators
    ...
    let skip = app
        .scroll_offset
        .min(app.indicators.len().saturating_sub(1));
    let ind_items: Vec<ListItem> = app
        .indicators
        .iter()
        .rev()
        .skip(skip)
        ...
        .map(|i| {
            ...
            ListItem::new(vec![
                ...
                Line::from(vec![
                    Span::raw("  "),
                    Span::styled(
                        format!("sev={:.2}", i.severity),
                        Style::default().fg(sev_color),
                    ),
                    Span::raw(format!(" conf={:.2} ", i.confidence)),
                    Span::styled(&i.source, Style::default().fg(Color::DarkGray)),
                ]),
                Line::from(vec![Span::raw("  "), Span::raw(&i.description)]),
                Line::from(vec![
                    Span::raw("  "),
                    Span::styled("Time: ", Style::default().fg(Color::DarkGray)),
                    Span::raw(&i.timestamp),
                    Span::raw(" │ "),
                    Span::styled("Era: ", Style::default().fg(Color::DarkGray)),
                    Span::styled(&i.era_relevance, Style::default().fg(Color::Yellow)),
                ]),
            ])
        })
        .collect();

    let ind_list = List::new(ind_items).block(
        Block::default()
            .title(format!(" Indicators ({}) ", app.indicators.len()))
            .borders(Borders::ALL)
            .border_style(Style::default().fg(Color::Magenta)),
    );
    f.render_widget(ind_list, bottom_chunks[1]);

    // Right: Category breakdown (moved to bottom)
    let cat_items: Vec<ListItem> = app
        .category_risks
        .iter()
        .map(|c| {
    ...
    f.render_widget(cat_list, bottom_chunks[0]);

    // Right: Indicators (already rendered above)
}
...
fn render_apqc(f: &mut Frame, app: &App, area: Rect) {
    ...
    let algo_text = if let Some(ref status) = app.status {
        ...
        // Add actual signatures from status
        for sig in &status.apqc.signatures {
            lines.push(Line::from(vec![
                Span::raw("   "),
                Span::styled(format!("● {} ", sig), Style::default().fg(Color::Green)),
            ]));
        }
        ...
        // Add actual KEMs from status
        for kem in &status.apqc.kems {
            lines.push(Line::from(vec![
                Span::raw("   "),
                Span::styled(format!("● {} ", kem), Style::default().fg(Color::Green)),
            ]));
        }

        lines.push(Line::from(""));
        lines.push(Line::from(vec![
            Span::styled(" Rotation: ", Style::default().fg(Color::Cyan)),
            Span::styled(
                if status.apqc.rotation_pending {
                    format!(
                        "PENDING (block {})",
                        status.apqc.rotation_block.unwrap_or(0)
                    )
                } else {
                    "NONE".to_string()
                },
                Style::default().fg(if status.apqc.rotation_pending {
                    Color::Yellow
                } else {
                    Color::Green
                }),
    ...
    let algo_para = Paragraph::new(algo_text).block(
        Block::default()
    ...
    let sig_items: Vec<ListItem> = app
        .recent_batches
        .iter()
        .rev()
        .take(8)
        .map(|b| {
            ListItem::new(vec![
                ...
                Line::from(vec![
                    Span::raw("  ML-DSA:  "),
                    Span::styled(
                        &b.ml_dsa_sig[..32.min(b.ml_dsa_sig.len())],
                ...
                Line::from(vec![
                    Span::raw("  SLH-DSA: "),
                    Span::styled(
                        &b.slh_dsa_sig[..32.min(b.slh_dsa_sig.len())],
                        Style::default().fg(Color::Green),
                    ),
                    Span::raw("..."),
                ]),
                Line::from(vec![
                    Span::styled("  Time: ", Style::default().fg(Color::DarkGray)),
                    Span::raw(&b.timestamp),
                ]),
            ])
        })
        .collect();

    let sig_list = List::new(sig_items).block(
        Block::default()
...
fn render_sequencer(f: &mut Frame, app: &App, area: Rect) {
    ...
    // Top: Aegis-TEE Status
    let tee_status = if let Some(ref status) = app.status {
        if let Some(ref aegis) = status.aegis_tee {
            vec![
                Line::from(vec![
                    Span::styled(" Platform: ", Style::default().fg(Color::Cyan)),
                    Span::styled(&aegis.platform, Style::default().fg(Color::Green)),
                    Span::raw(" │ "),
                    Span::styled(" Enclave: ", Style::default().fg(Color::Cyan)),
                    Span::styled(&aegis.mrenclave, Style::default().fg(Color::Yellow)),
                ]),
                Line::from(vec![
                    Span::styled(" Mempool Size: ", Style::default().fg(Color::Cyan)),
                    Span::raw(format!("{}", aegis.mempool_size)),
                    Span::raw(" │ "),
                    Span::styled(" Asset Protection: ", Style::default().fg(Color::Cyan)),
                    Span::styled(
                        if aegis.asset_protection_enabled {
                            "ENABLED"
                        } else {
                            "DISABLED"
                        },
                        Style::default().fg(if aegis.asset_protection_enabled {
                            Color::Green
                        } else {
                            Color::DarkGray
                        }),
                ...
                Line::from(vec![
                    Span::styled(" Phala Redundancy: ", Style::default().fg(Color::Cyan)),
                    Span::styled(
                        if aegis.phala_redundancy_enabled {
                            "ENABLED"
                        } else {
                            "DISABLED"
                        },
                        Style::default().fg(if aegis.phala_redundancy_enabled {
                            Color::Green
                        } else {
                            Color::DarkGray
                        }),
            ...
            vec![Line::from("Aegis-TEE status not available")]
        }
    } else {
        vec![Line::from("Awaiting connection...")]
    };

    let tee_para = Paragraph::new(tee_status).block(
        Block::default()
            .title(" Aegis-TEE Status ")
            .borders(Borders::ALL)
            .border_style(Style::default().fg(Color::Yellow)),
    );
    f.render_widget(tee_para, chunks[0]);

    // Bottom: Sequencer details
    let bottom_chunks = Layout::default()
        .direction(Direction::Horizontal)
        .constraints([Constraint::Percentage(50), Constraint::Percentage(50)])
        .split(chunks[1]);

    // Left: Sequencer Stats
    let seq_stats = if let Some(ref status) = app.status {
        vec![
            Line::from(vec![
                Span::styled(" Mempool: ", Style::default().fg(Color::Cyan)),
                Span::raw(format!("{}", status.sequencer.mempool_size)),
            ]),
            Line::from(vec![
                Span::styled(" Ordered Queue: ", Style::default().fg(Color::Cyan)),
                Span::raw(format!("{}", status.sequencer.ordered_queue)),
            ]),
            Line::from(vec![
                Span::styled(" Batches: ", Style::default().fg(Color::Cyan)),
                Span::raw(format!("{}", status.sequencer.batch_count)),
            ]),
            Line::from(""),
            Line::from(vec![
                Span::styled(" TEE Platform: ", Style::default().fg(Color::Cyan)),
                Span::styled(
                    &status.sequencer.tee_platform,
                    Style::default().fg(Color::Green),
                ),
            ]),
            Line::from(vec![
    ...
    let stats_para = Paragraph::new(seq_stats).block(
        Block::default()
            .title(" Sequencer Stats ")
            .borders(Borders::ALL)
            .border_style(Style::default().fg(Color::Yellow)),
    );
    ...
    let tx_list = List::new(tx_items).block(
        Block::default()
            .title(format!(" Transactions ({}) ", app.pending_txs.len()))
            .borders(Borders::ALL)
            .border_style(Style::default().fg(Color::Yellow)),
    );
    f.render_widget(tx_list, bottom_chunks[1]);
}
...
fn render_chain(f: &mut Frame, app: &App, area: Rect) {
    ...
    // Top: Chain Status
    let chain_status = if let Some(ref status) = app.status {
        let risk_color = if status.chain.risk_score < 3000 {
            Color::Green
        } else if status.chain.risk_score < 6000 {
            Color::Yellow
        } else {
            Color::Red
        };
        vec![
            Line::from(vec![
                Span::styled(" Height: ", Style::default().fg(Color::Cyan)),
                Span::styled(
                    format!("{:>6}", status.chain.height),
                    Style::default()
                        .fg(Color::Green)
                        .add_modifier(Modifier::BOLD),
                ),
                ...
                Span::styled(" Risk Score: ", Style::default().fg(Color::Cyan)),
                Span::styled(
                    format!("{:>5}", status.chain.risk_score),
                    Style::default().fg(risk_color),
                ),
            ]),
            Line::from(vec![
                Span::styled(" Active Signatures: ", Style::default().fg(Color::Cyan)),
                Span::raw(status.chain.algorithm_set.signatures.join(", ")),
            ]),
            Line::from(vec![
                Span::styled(" Active KEMs: ", Style::default().fg(Color::Cyan)),
                Span::raw(status.chain.algorithm_set.kems.join(", ")),
            ]),
        ]
    } else {
        vec![Line::from("Awaiting connection...")]
    ...
    let status_para = Paragraph::new(chain_status).block(
        Block::default()
            .title(" Chain Status ")
            .borders(Borders::ALL)
...
fn render_all(f: &mut Frame, app: &App, area: Rect) {
    ...
    // QRM mini
    let qrm_items: Vec<ListItem> = app
        .indicators
        .iter()
        .rev()
        .take(8)
        .map(|i| {
            ListItem::new(Line::from(vec![
                Span::styled(
                    format!("[{}]", &i.category[..3].to_uppercase()),
                    Style::default().fg(Color::Magenta),
                ),
                Span::raw(" "),
        ...
        .collect();
    let qrm_list = List::new(qrm_items).block(
        Block::default()
            .title(" QRM ")
            .borders(Borders::ALL)
            .border_style(Style::default().fg(Color::Magenta)),
    ...
    let apqc_para = Paragraph::new(apqc_text).block(
        Block::default()
            .title(" APQC ")
            .borders(Borders::ALL)
            .border_style(Style::default().fg(Color::Blue)),
    ...
    let seq_para = Paragraph::new(seq_text).block(
        Block::default()
            .title(" SEQ ")
            .borders(Borders::ALL)
            .border_style(Style::default().fg(Color::Yellow)),
    ...
    let chain_items: Vec<ListItem> = app
        ...
        .rev()
        .take(6)
        .map(|b| ListItem::new(Line::from(format!("#{} txs={}", b.height, b.tx_count))))
        .collect();
    ...
    let qvm_para = Paragraph::new(qvm_text).block(
        Block::default()
            .title(" QVM ")
            .borders(Borders::ALL)
            .border_style(Style::default().fg(Color::Magenta)),
        ...
        if event::poll(Duration::from_millis(50))? {
            if let Event::Key(key) = event::read()? {
                match key.code {
                    ...
                    KeyCode::Char('h') => {
                        let _ = cmd_tx
                            .send(r#"{"command":"inject_high"}"#.to_string())
                            .await;
                        app.log(
                            LogLevel::Warn,
                            "CMD",
                            "Sent INJECT HIGH THREAT command".to_string(),
                        );
                    }
